...
CATEGORIES = {
    ...
    "2. 水着（リゾート）": {"en": "Resort swimwear", "back_prompt": "healthy skin glow, summer light"},
    "3. 部屋着（リラックス）": {"en": "Silk night-fashion", "back_prompt": "soft beauty light"},
    "4. オフィス（スーツ）": {"en": "Business professional attire", "back_prompt": "studio look"},
    "5. コスチューム": {"en": "Themed costume", "back_prompt": "professional strobe"},
    "6. 夜の装い（ドレス）": {"en": "Sophisticated evening gown", "back_prompt": "dramatic lighting, facial fill-light"}
}

# --- 2. ユーティリティ (辞書形式による安全版) ---
def generate_with_retry(client, contents, prompt, max_retries=2):
    for attempt in range(max_retries + 1):
        try:
            # クラス名のAttributeErrorを防ぐため、設定を辞書(dict)形式で渡す
            config_dict = {
...
def show_kisekae_ui():
    ...
    if run_btn and source_img:
        ...
        if isinstance(res_data, bytes):
            ...
            st.session_state.wardrobe_task = f"Strictly apply the textile design from IMAGE 2. {cloth_detail}."
