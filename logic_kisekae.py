...
# --- 2. 黄金律に基づく生成エンジン (SDK 0.6.0 準拠) ---
def generate_with_retry(client, contents, prompt, max_retries=2):
    """503エラーのリトライ機能を備えたコア生成関数"""
    for attempt in range(max_retries + 1):
        try:
            # SDK 0.6.0 の厳格なバリデーションに適合した設定
...
def show_kisekae_ui():
    ...
    st.header("✨ AI KISEKAE Main System (v2.80)")
    ...
    with st.sidebar:
        ...
        cloth_detail = st.text_input("衣装仕様書", placeholder="例：サテン、シルク")
    ...
    if run_btn and source_img:
        ...
        # Step 1: 衣装アンカー (2ステップロジック)
        status.info("🕒 Step 1/2: 衣装設計図（アンカー）を抽出中...")
        anchor_prompt = f"Professional product photography of {CATEGORIES[cloth_main]['en']} material. {cloth_detail}. Textile scan quality, neutral background."
        ...
        if isinstance(res_data, bytes):
            ...
            st.session_state.wardrobe_task = f"Strictly replicate the textile design and pattern from IMAGE 2. {cloth_detail}."
