...
# --- 1. システム設定 (ver 2.55: Face Fidelity Update) ---
VERSION = "2.55"
...
# カテゴリー定義 (裏プロンプトに顔のライティング指示を追加)
CATEGORIES = {
    "1. 私服（日常）": {"en": "Casual everyday Japanese fashion", "back_prompt": "natural skin texture, soft daylight, high facial clarity"},
    "2. 水着（ビーチ）": {"en": "High-end stylish beachwear", "back_prompt": "healthy skin glow, sun-kissed facial highlights, crisp detail"},
    "3. 部屋着（リラックス）": {"en": "Soft lounge wear, silk or knit lingerie-style", "back_prompt": "warm rim lighting, soft beauty face light, cinematic intimacy"},
    "4. オフィス（スーツ）": {"en": "Elegant business professional attire", "back_prompt": "clean corporate lighting, sharp focus on facial features, professional"},
    "5. コスチューム": {"en": "High-quality themed costume", "back_prompt": "studio strobe lighting, flawless skin render, meticulous detail"},
    "6. 夜の装い（ドレス）": {"en": "Sophisticated evening gown", "back_prompt": "luxury bokeh, dramatic but soft facial lighting, glamorous sheen"}
}
...
def generate_image_by_text(client, pose_text, identity_part, anchor_part, wardrobe_task, bg_prompt, enable_blur, cat_key):
    """【ver 2.55】顔の幾何学的再現性を極限まで高めたプロンプト"""
    cat_info = CATEGORIES[cat_key]
    prompt = (
        f"STRICT PHYSICAL FIDELITY: ABSOLUTE IDENTITY LOCK.\n"
        f"1. FACE FIDELITY (IMAGE 1): Replicate the EXACT facial landmarks, eye shape, nose structure, and chin contour of the woman in IMAGE 1. 1:1 pixel-level matching of facial features. Do not alter her identity.\n"
        f"2. BODY FIDELITY (IMAGE 1): [FIXED_IDENTITY] Match the exact body mass, curves, and shoulder width of IMAGE 1.\n"
        f"3. POSE & COMPOSITION: {pose_text}.\n"
        f"4. WARDROBE (IMAGE 2): {wardrobe_task}\n"
        f"5. RENDER: {bg_prompt}, {cat_info['back_prompt']}, 85mm lens, 8k resolution, ultra-detailed skin texture, professional studio lighting, lips sealed."
    )
...
if run_btn and source_img:
    ...
    with st.spinner("衣装設計図（アンカー）を構築中..."):
        anchor_prompt = f"Studio catalog photo of the {CATEGORIES[cloth_main]['en']}. Specs: {cloth_detail}. Isolated view, high definition."
