...
def generate_image_by_text(client, pose_text, identity_part, anchor_part, wardrobe_task, bg_prompt, enable_blur, cat_info):
    """【ver 2.5 改良】衣装一貫性の死守ロジック"""
    full_prompt = (
        f"[CORE RULE: ABSOLUTE VISUAL FIDELITY]\n"
        f"1. IDENTITY (IMAGE 1): Replicate the exact face and body of the woman in IMAGE 1. STRICT FIDELITY to 2.5/2.43 proportions.\n"
        f"2. WARDROBE (IMAGE 2): **STRICTLY COPY** the outfit from IMAGE 2. Every detail, color, and texture must be IDENTICAL to IMAGE 2. No creative variations allowed. Same photo shoot.\n"
        f"3. POSE: {pose_text}.\n"
        f"4. ENVIRONMENT: {bg_prompt}. {cat_info['back_prompt']}. 85mm lens portrait.\n"
        f"Ensure all images maintain 100% clothing consistency."
    )
